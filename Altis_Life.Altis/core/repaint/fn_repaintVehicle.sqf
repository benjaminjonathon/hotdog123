...
if ((getPlayerUID player) != _vehOwner) exitWith {hintSilent "You aren't the owner!";_ui = "osefStatusBar" call BIS_fnc_rscLayer;_ui cutRsc["osefStatusBar","PLAIN"];};
		...
		if(life_interrupted) exitWith {life_interrupted = false; titleText["Aborted","PLAIN"]; life_action_inUse = false;_ui = "osefStatusBar" call BIS_fnc_rscLayer;_ui cutRsc["osefStatusBar","PLAIN"];};
		if(player != vehicle player) exitWith {titleText["You have to get out of the vehicle in order to paint it can, to paint it can!","PLAIN"];_ui = "osefStatusBar" call BIS_fnc_rscLayer;_ui cutRsc["osefStatusBar","PLAIN"];};
