...
switch (_code) do {
	...
	case 74: {
		if(!_shift && !_alt) then {
			_handled = true;
			if(soundVolume == 0.15) then {
				0.5 fadeSound 1;
				5 cutText ["Earplugs removed", "PLAIN"];
			} else {
				0.5 fadeSound 0.15;
				5 cutText ["Put in earplugs", "PLAIN"];
			};
		};
	};
	//Interaction key (default is Left Windows, can be mapped via Controls -> Custom -> User Action 10)
...
/*
	//cop tazer control non lethal
	case 79: {
		switch (playerSide) do {
			case west: {life_leathaltazer = false; hint "Tazer is non lethal";};
		};
	};
	
	//cop tazer control Lethal
	case 80: {
		switch (playerSide) do {
			case west: {life_leathaltazer = true; hint "Tazer is  lethal";};
		};
	};
*/	
	//Restraining (Shift + R)
